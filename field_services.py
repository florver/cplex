...
def add_constraint_matrix(my_problem, data):
    ...
    for n in range(data.ordenes):
      ...
      for j in range(len(data.beneficio)):
    ...
    #for j in range(trabajadores):
      ...
      for j in range(data.trabajadores):
            for d in range(data.dias):
              variables_restriccion = []
    ...
    for n in range(data.ordenes):
      ...
      for j in range(data.trabajadores):
        ...
        for k in range(len(data.costos)):
          ...
          for jj in range(data.trabajadores):
            if j != jj:
              variables_costos_2 = []
      ...
      for j in range(data.trabajadores):
        ...
        for d in range(data.dias):
          for t in range(data.turnos):
            for n in range(data.ordenes):
              variables_restriccion.append('v'+'_'+str(j)+'_'+str(d)+'_'+str(t)+'_'+str(n))
              values = list([[1]*len(variables_restriccion), [-1]*len(variables_costos)])
...
def populate_by_row(my_problem, data):
    ...
    for j in range(data.cantidad_trabajadores): 
      for d in range(data.dias)
        for t in range(data.turnos):
          for n in data.beneficio:
            coeficientes_beneficio.append(beneficio.value)
    coeficientes_costo = list(np.tile(costos, data.cantidad_trabajadores))
    ...
    for j in range(data.trabajadores): 
      for d in range(data.dias):
        for t in range(data.turnos):
          for n in range(len(data.beneficio)):
            variables_beneficios.append('v'+'_'+str(j)+'_'+str(d)+'_'+str(t)+'_'+str(item))
    ...
    my_problem.variables.add(obj = coeficientes_funcion_objetivo, lb =[[0]*len(coeficientes_beneficio),[0]*len(coeficientes_costo)], ub = [[1]*len(coeficientes_beneficio),list(np.tile(trozos, data.cantidad_trabajadores))]
                             , types=[['B']*len(coeficientes_beneficio),['I']*len(coeficientes_costo)], variable_names = list([variables_beneficios,variables_costos]))
    ...
    my_problem.variables.add(obj = [0.0] * len(variable_z), lb = [0]*len(variable_z), ub = [1]*len(variable_z), types= ['B']*len(variable_z), variable_names = variable_z)
    ...
    my_problem.variables.add(obj = [0.0] * len(variable_delta), lb = [0]*len(variable_delta), ub = [1]*len(variable_delta), types= ['B']*len(variable_delta), variable_names = variable_delta)
    ...
    my_problem.variables.add(obj = [0.0] * len(variable_lambda), lb = [0]*len(variable_lambda), ub = [1]*len(variable_lambda), types= ['B']*len(variable_lambda), variable_names = variable_lambda)
    ...
    my_problem.variables.add([0.0] * len(variables_w), lb = [0]*len(variables_w), ub = [1]*len(variables_w), types= ['B']*len(variables_w), variable_names = variables_w)
